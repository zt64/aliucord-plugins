...
class PluginSettings(private val settings: SettingsAPI) : BottomSheet() {
    ...
    override fun onViewCreated(view: View, bundle: Bundle?) {
        ...
        addView(Util.createSwitch(ctx, "Show Selected", "Whether selected channels should be visible even if the category is collapsed").apply {
            isChecked = settings.showSelected
            setOnCheckedListener { settings.showSelected = it }
        })
        addView(Util.createSwitch(ctx, "Show Unread", "Whether unread channels should be visible even if the category is collapsed").apply {
