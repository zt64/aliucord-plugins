...
open class SpannedGridLayoutManager(
    ...
    open val firstVisiblePosition: Int
        get() = if (childCount == 0) 0 else getPosition(getChildAt(0)!!)
    ...
    class SavedState(val firstVisibleItem: Int) : Parcelable {
        ...
        override fun writeToParcel(dest: Parcel, flags: Int) = dest.writeInt(firstVisibleItem)
...
open class RectsHelper(
    ...
    private val size: Int
        get() = if (orientation == SpannedGridLayoutManager.Orientation.VERTICAL) {
    ...
    fun findRect(position: Int, spanSize: SpanSize): Rect =
        rectsCache[position] ?: findRectForSpanSize(spanSize)
    ...
    fun findPositionsForRow(rowPosition: Int): Set<Int> = rows[rowPosition] ?: emptySet()
