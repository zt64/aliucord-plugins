...
class PluginSettings(private val settings: SettingsAPI) : SettingsPage() {
    ...
    override fun onViewBound(view: View) {
        ...
        addView(
            createCheckedSetting(
                "Auto-hide controls",
                "Hide the top and bottom bar after a delay",
                "autoHideControls",
                true
            ).apply {
                setOnCheckedListener {
        ...
        val radioGroup = RadioGroup(ctx).apply {
            visibility =
                if (settings.getBool("immersiveModeState", true)) View.VISIBLE else View.GONE
        }
        ...
        addView(
            createCheckedSetting(
                "Hide back button",
                "Hide the back button from the toolbar",
                "hideBackButton",
                false
            )
        )
        addView(
            createCheckedSetting(
                "Bottom toolbar",
                "Moves the bar to the bottom of the screen for easier one handed use. Only supports images",
                "bottomToolbar",
                false
            )
        )
        addView(
            createCheckedSetting(
                "Remove zoom limit",
                "Removes the zoom limit for images",
                "removeZoomLimit",
                true
            )
        )
        addView(
            createCheckedSetting(
                ...
                "Shows the open in browser button on the toolbar",
                "showOpenInBrowser",
                true
            )
        )
        ...
        val downloadDir = TextView(ctx, null, 0, R.i.UiKit_TextView_Subtext).apply {
            text = "Current Directory: " + settings.getString(
                "downloadDir",
